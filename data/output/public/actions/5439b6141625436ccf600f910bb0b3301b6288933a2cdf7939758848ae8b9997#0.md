...
Vote: NEEDS_MORE_INFO. For this TreasuryWithdrawals (active), critical information needed for a responsible directional vote is still missing. Decisive reason: if claims cannot be tied to replayable evidence, abstaining avoids overclaiming certainty. Weighed against it: the proposal substantiates "The proposal says Blockfrost adoption in the Cardano Foundation developer ecosystem survey rose from 39.3% in 2022 to 71.5% in 2025, with links to the survey pages." and clears the evidence gates. What would enable a directional vote: Provide: cost comparables, budget proportionality, or sustainability evidence; Provide independent evidence for: Blockfrost provides a hosted REST interface that lets developers read from and interact with Cardano without running their own node. Residual uncertainty: Treasury flow signal is in stressed regime (total inflow (tau + donations) vs enacted withdrawals). Confidence context: score 0.20; confidence 74.0%.
...
- Provide independent evidence for: Blockfrost provides a hosted REST interface that lets developers read from and interact with Cardano without running their own node.
